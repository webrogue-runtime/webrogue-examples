...
//	device_layers.push_back( "VK_LAYER_LUNARG_standard_validation" );			// depricated
    ...
    glfwInitVulkanLoader(vk_icdGetInstanceProcAddr);
	// don't forget to initialize glfw!
	glfwInit();
    // PFN_vkGetInstanceProcAddr vkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr) vk_icdGetInstanceProcAddr(nullptr, "vkGetInstanceProcAddr");
    ...
    #define xstr(s) str(s)
    #define str(s) #s
    #define LOAD_VK_PROC(proc) PFN_##proc proc = (PFN_##proc) glfwGetInstanceProcAddress(instance, str(proc))
    LOAD_VK_PROC(vkEnumeratePhysicalDevices);
    LOAD_VK_PROC(vkGetPhysicalDeviceQueueFamilyProperties);
    LOAD_VK_PROC(vkCreateDevice);
    LOAD_VK_PROC(vkDestroySurfaceKHR);
    LOAD_VK_PROC(vkDestroyDevice);
    LOAD_VK_PROC(vkDestroyInstance);
